...
export const useUserStore = create<UserState>((set, get) => ({
  ...
  addUser: async (userData) => {
    try {
      ...
      const { data, error } = await supabase
        .from('clients')
        .insert({
          // The ID is autogenerated by Auth if registering, or by DB if manual
          name: userData.name,
  ...
  updateUser: async (id, changes) => {
    try {
      ...
      if (changes.isTest !== undefined) dbChanges.is_test = changes.isTest;
      
      // Remove fields not present in DB or readonly
